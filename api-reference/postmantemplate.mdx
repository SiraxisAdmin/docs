...
<Info>
  Copy the following JSON into Postman to access your container data.
  - Obtain a bearer token from the authentication token endpoint, using the required API key in the `SlyceIO-Api-Key` header.
  - Replace the `containerDefinitionId` and `documentId` variables with the required values.
  - Ensure any `partitionKeys` query parameter values are **url encoded**.
</Info>
