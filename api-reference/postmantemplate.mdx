...
<Info>
  ...
  - Obtain a bearer token from the authentication token endpoint using the `SlyceIO-Api-Key` header.
  - Replace the `containerDefinitionId` and `documentId` variables with the required values.
  - Ensure `partitionKeys` query parameter values are **url encoded**.
</Info>
